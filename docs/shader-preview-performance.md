...
### Production build mismatch diagnosed 2026-09-09

Live Vercel inspection now confirms `general-translation/glyphfield` is linked
to `Kevin-Liu-01/Glyphfield`, production branch `main`. The September 7 source
policy incident above is historical; the September 9 Git builds accept this
source. Do not infer current release state from that old incident.

The toolbar release `3fe722c` reached this project but deployment
`glyphfield-puo3kbyca-general-translation.vercel.app` failed at dependency
installation with `ERR_PNPM_LOCKFILE_CONFIG_MISMATCH`. Vercel selected pnpm 10
from the lockfile version/project age, while the patched-dependency lockfile was
generated by pnpm 11.15.1. The public alias consequently kept serving `b890e13`
from `glyphfield-cdi80yyf4-general-translation.vercel.app`.

Pin `packageManager` to `pnpm@11.15.1` and enable
`ENABLE_EXPERIMENTAL_COREPACK=1` in the Vercel project's build environments,
following [Vercel's Corepack setup](https://vercel.com/docs/builds/configure-a-build#corepack).
Keep frozen installs enabled. Release verification must check the build result,
the public alias's source commit, and both live Studio toolbar layouts.

### Preview release published 2026-09-06 (verification receipt)
