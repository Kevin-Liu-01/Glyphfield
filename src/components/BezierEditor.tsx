...
export default function BezierEditor({ compact = false, curve, onChange }: BezierEditorProps) {
  ...
  return (
    <div className={`bezier-editor flex flex-col border-t border-border ${compact ? 'gap-2 pt-3' : 'gap-3 pt-4'}`}>
      <div className={compact ? 'grid gap-1' : 'flex items-center justify-between gap-4'}>
        <span className='text-sm font-medium'>
          <T>Cubic bezier</T>
        </span>
        <code className={`font-mono text-xs text-muted-foreground ${compact ? 'text-[8px] leading-tight' : ''}`}>
          {curve.map((value) => value.toFixed(2)).join(', ')}
        </code>
      </div>
      <div className={`grid ${compact ? 'grid-cols-1 gap-2' : 'grid-cols-[120px_1fr] gap-4'}`}>
        <svg
          aria-label='Cubic bezier preview'
          className={`${compact ? 'mx-auto size-[104px]' : 'size-[120px]'} border border-border bg-background`}
          viewBox='0 0 120 120'
        >
