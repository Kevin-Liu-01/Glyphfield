  ...
  return (
    <PreviewSurface disabled={disabled}>
      <div className={`mx-auto grid min-h-[420px] max-w-2xl content-center gap-5 ${spacing}`}>
        ...
        {pattern === 'checkboxes' ? ['Automatic updates', 'Product announcements', 'Weekly summary'].map((item, index) => <label className='flex items-center gap-3 border border-border p-4 text-sm' key={item}><StudioCheckbox checked={index === 0 ? checked : index === 1} onChange={index === 0 ? (event) => setChecked(event.target.checked) : undefined} readOnly={index !== 0} />{gt(item)}</label>) : null}
        {pattern === 'radios' ? ['Public', 'Team only', 'Private'].map((item, index) => <label className='flex items-center gap-3 border border-border p-4 text-sm' key={item}><StudioRadio checked={activeIndex === index} name='visibility' onChange={() => setActiveIndex(index)} />{gt(item)}</label>) : null}
  ...
  return (
    <PreviewSurface disabled={disabled}>
      <div className={`mx-auto grid min-h-[420px] max-w-3xl content-center gap-4 ${spacing}`}>
        ...
        {pattern === 'progress' ? <div className='border border-border p-6'><div className='flex justify-between text-sm'><span><T>Generating assets</T></span><span>{progress}%</span></div><div className='mt-4 h-2 overflow-hidden rounded-full bg-muted'><div className='h-full bg-foreground transition-[width]' style={{ width: `${progress}%` }} /></div><StudioRange aria-label={gt('Progress')} className='mt-6' max={100} min={0} onChange={(event) => setProgress(Number(event.target.value))} value={progress} /></div> : null}
      </div>
    </PreviewSurface>
  );
}
