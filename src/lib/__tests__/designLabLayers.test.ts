...
describe('Playground optional layers', () => {
  ...
  it('moves artboards from a full-width drag handle with keyboard parity', () => {
    ...
    expect(designLab).toContain('translateCanvasFrame(artboard, { deltaX, deltaY, minX: -Infinity, minY })');
    expect(designLab).not.toContain('Math.ceil(36 / Math.max(0.01, artboardScale))');
    ...
    expect(designLab).toContain('navigationItems={workspaceArtboards.flatMap');
  ...
  it('renders previews and exports from the same logical text geometry', () => {
    ...
    expect(designLab).toContain('const fontSize = resolveDesignLabFontSize(layer, canvasHeight) * width / canvasWidth;');
    expect(designLab).toContain('resolveDesignLabFontSizeCqw(textLayer, canvasDimensions)');
    expect(designLab).toContain('resolveDesignLabFontSizeCqw(layer, snapshot.dimensions)');
    expect(designLab).not.toContain('getComputedStyle(previewElement)');
    expect(designLab).not.toContain('fitContentHeight');
    expect(designLab).toContain('...canvasTextTypography({');
    expect(designLab).toContain('layoutNativeCanvasText({');
    expect(designLab).not.toContain('layoutCanvasTextBlock(');
