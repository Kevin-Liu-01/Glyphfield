...
void main() {
  ...
  {
    description: 'Flowing chrome bands with spectral separation and sharp moving highlights.',
    fragmentSource: fragment(`
void main() {
  vec2 uv = gl_FragCoord.xy / u_resolution.xy;
  vec2 p = uv * 2.0 - 1.0;
  p.x *= u_resolution.x / u_resolution.y;
  float t = u_time * 0.8;
  float distortion = sin(p.x * 4.0 + t) * 0.3 + sin(p.y * 6.0 - t * 0.7) * 0.22;
  float flow = (p.x * 0.34 + p.y * 0.94) * 12.0 + distortion + t * 2.0;
  float bands = sin(flow) * 0.5 + 0.5;
  float metal = smoothstep(0.08, 0.92, bands);
  float shine = pow(max(0.0, sin(flow + 0.55)), 18.0);
  float redShift = sin(flow + 0.22) * 0.5 + 0.5;
  float blueShift = sin(flow - 0.25) * 0.5 + 0.5;
  vec3 color = mix(u_color_a * 0.08, mix(u_color_a, u_color_b, metal), 0.88);
  color.r += redShift * 0.14;
  color.b += blueShift * 0.18;
  color += shine * 0.72;
  gl_FragColor = vec4(color, 1.0);
}`),
    id: 'liquid-metal',
