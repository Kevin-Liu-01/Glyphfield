...
4. Fetch `/api/catalog` when selecting a deterministic Surface Lab recipe; its `surfaceLibrary` contains every preset and physical control.
...
10. Save raw SVG or extract the artifact from its JSON envelope.
