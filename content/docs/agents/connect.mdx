...
8. Fetch `GET /api/generate` immediately before constructing a request and validate against its `requestSchema` when possible.
...
curl -fsS "$BASE_URL/api/generate" | jq '{kinds: (.kinds | keys), requestSchema}'
```
