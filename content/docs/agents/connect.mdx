...
4. Fetch `/api/materials` when selecting a shader or editing shared material controls.
5. Fetch `/api/identities` when using any built-in preset.
6. Fetch `/api/elements` when choosing a brand application.
7. Fetch `GET /api/generate` immediately before constructing a request.
8. `POST application/json` to `/api/generate`.
9. Save raw SVG or extract the artifact from its JSON envelope.
...
When a task requires an artifact that the public generation API cannot fully express, open the matching Studio tool and select **Code**. Read the current source before editing it, apply the change, resolve any inline validation error, and visually verify the canvas before downloading.

Do not paste a Studio source document into `POST /api/generate`. The drawer is the browser-local document for an open tool; the generation route is a separate versioned HTTP contract. See [Browser workflow](/docs/agents/browser-workflow), [Source code editing](/docs/studio/source-editing), and [Source formats](/docs/reference/source-formats).
