...
Browser projects persist in `localStorage`. The Browser API reads and mutates that same active project state. Stateless HTTP requests do not silently mutate projects; generated apply-ready documents must be passed explicitly to `applySource`.
