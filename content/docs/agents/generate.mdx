...
- `document`: exact Design Lab source version 3.
...
Use the HTTP contract on this page for deterministic remote generation. `design-sequence` is deliberately a bridge: it emits an exact Studio source document for programmatic browser rendering. For every other tool, use the active `window.glyphfield.studio` API to read/apply source or operate visible controls.
