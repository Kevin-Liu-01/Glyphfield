...
WebGL and motion capture use the browser renderer. Agents can operate them through `window.glyphfield.studio`; Design Lab exposes PNG/JPG, GIF, MP4, and shader-sequence GIF/MP4 export actions. Use unified `design.export` with `download: true` to render and save in one call.
