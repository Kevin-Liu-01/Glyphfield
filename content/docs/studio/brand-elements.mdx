...
<DocsMedia
  alt='General Translation Brand elements workspace with application categories and a rendered artifact'
  caption='One identity resolved across production applications'
  meta='Digital · product · editorial · physical'
  src='/screenshots/studio-gt-brand-elements-dark-2026.png'
/>
...
Select **File → Edit source code** to edit the selected element override as JSON. The drawer contains only the mutable override for that application; the catalog record and identity-derived defaults remain separate inputs.
