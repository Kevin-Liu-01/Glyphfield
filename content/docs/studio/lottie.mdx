...
The built-in library starts with seven original motion systems: signal relay, decision routing, source synthesis, layer assembly, quality scanning, network orbit, and endpoint delivery. These are broad visual ideas rather than product-specific stories, so they can adapt to launches, workflows, data, teams, content, or infrastructure. Each scene is an editable, loop-safe Lottie document built from vector paths, typography, restrained linework, and one brand accent.
