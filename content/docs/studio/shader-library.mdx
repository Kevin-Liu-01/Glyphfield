...
Design Lab exports PNG, JPG, GIF, MP4, and shader-sequence GIF/MP4. Animation composites selected shader backgrounds into its GIF. Live WebGL output uses the browser renderer and is programmatically available through `window.glyphfield.studio`; Surface Lab also provides deterministic static SVG backgrounds and physical-material recipes.
