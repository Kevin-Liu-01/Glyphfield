...
Shader time, pause state, size, colors, and every common setting are part of the host document. Design Lab also saves a lossless PNG for captured appearances, alongside editable native frame state. Its time explorer is not a recording of every live frame and does not imply that every shader has a finite loop. Fluid Energy can be captured as a still, but its history-dependent simulation cannot be reconstructed from a time value; restarting motion starts a new simulation. Read each catalog record's `motion` capabilities before choosing seek or loop behavior.
