...
Every generator uses the same [editor systems](/docs/studio/editor-systems) for color, canvas zoom, selection, layers, alignment, assets, persistent drafts, and source editing. This makes an interaction learned in one tool available everywhere it applies. The shared [Shader Library](/docs/studio/shader-library) adds more than 140 quality-sorted live materials and presets to Material and Animation, while Surface Lab stays focused on deterministic static backgrounds.
...
| Boards | Moodboard |
| Motion | Animation, Lottie, Material, shared Shader Library |
| Foundations | Brand settings, Surface Lab, Color tokens, Typography |
