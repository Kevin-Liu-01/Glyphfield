...
<DocsMediaGrid>
  <DocsMedia alt='Design Lab showing shared library, canvas viewport, inspector, and layer dock systems' caption='Layered editor systems' meta='Selection · zoom · source · persistence' src='/screenshots/studio-gt-design-lab-dark-2026.png' />
  <DocsMedia alt='Brand Elements showing identity-aware artifact controls' caption='Identity-aware systems' meta='Shared assets · color · output' src='/screenshots/studio-gt-brand-elements-dark-2026.png' />
</DocsMediaGrid>
...
| --- | --- | --- | --- |
| `StudioToolHeader` / `StudioFileMenu` | Shared header hierarchy, explicit file operations, and advanced source access | Host supplies existing action callbacks | Every source-enabled tool |
| `StudioActionMenu` | Named, keyboard-operable toolbar menus built on the shared context menu | Trigger owns open state; active workspace owns the portal | Project actions and File menus |
| `LabInspectorSection` | Icon-labeled, collapsible inspector sections; children stay mounted | Local presentation state only | Shared libraries and inspectors |
...
| State | Behavior |
| --- | --- |
| Resting | Compact row with the current swatch, label, normalized HEX, and optional opacity |
| Open | Full picker with HEX, OKLCH, hue, saturation/brightness, and optional opacity; `compact={false}` remains available for dedicated color-editing surfaces |
| Screen eyedropper active | **Pick screen color** (dropper icon) samples any visible screen pixel, including outside the browser; selecting commits one normalized HEX edit without changing opacity; Escape cancels |
...
| Dragging | Coalesces picker and preview updates to animation frames; the native slider thumb stays responsive and release commits the final value immediately |
...
The screen eyedropper is shared by compact and expanded color pickers. It requires
a browser with the EyeDropper API on HTTPS or localhost; unsupported browsers show
a disabled button with an explanation and retain the other color controls. Closing
the picker or leaving its editor cancels an unfinished sample.

Browser API control discovery exposes the uniquely labeled `{color label} pick screen color`
button. Screen sampling requires a real user gesture and native screen selection;
agents should use the existing HEX or OKLCH controls for deterministic edits and
re-read the source to verify completion. This browser-only interaction does not change
the source schema, tool catalog, or deterministic HTTP generation contract.
...
See [Source code editing](/docs/studio/source-editing), [Source format reference](/docs/reference/source-formats), and [Studio Browser API](/docs/reference/browser-api).
