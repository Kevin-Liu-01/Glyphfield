...
Every source-enabled Studio tool can be edited visually or structurally. **File → Edit source code** opens the document beside the live canvas; `window.glyphfield.studio` exposes the same read/apply boundary to agents.
...
2. Open **File → Edit source code** from the tool action area.
...
For file-based sharing, download **Project file** from the export controls and reopen it with **Open project**. The `.glyphfield.json` file uses this same CanvasDocument envelope, with local images and saved frame assets embedded. See [Project files](/docs/studio/project-files).
