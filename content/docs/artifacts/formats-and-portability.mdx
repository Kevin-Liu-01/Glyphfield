...
Design Lab freezes the currently visible native shader frame before composing a PNG or JPG at the selected output dimensions. The export leaves playback paused and does not create stored shader-frame assets. To preserve an editable appearance checkpoint as well as a rendered artifact, explicitly capture the shader frame and retain the portable CanvasDocument source.
...
Design Lab GIF/MP4 explicitly samples motion from the current frame anchor and restores that paused pose afterward, without persisting captured-frame assets. A saved PNG preserves appearance, not simulation history: stateful Fluid supports exact still export, but timestamp-sampled motion export is unavailable.
