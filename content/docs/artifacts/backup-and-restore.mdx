...
| Captured shader frames | Content-addressed PNG Blobs in IndexedDB | Yes when PNG bytes are embedded in CanvasDocument assets | Capture the intended look and keep the complete portable source, not only its frame timestamp |
...
// Freeze and persist the actual visible shader appearance before backing it up.
...
Captured PNGs preserve the approved still even when the shader provider, GPU, or browser changes. Keep the editable recipe too. Fluid Energy requires prior simulation state to continue identically; its PNG preserves the still, not the fluid's history.
