...
Before downloading, select **Code** when you need to inspect or precisely edit the artifact’s working source. Applying source updates the same state used by preview and export. Download the rendered artifact through the normal output action; the drawer itself is not an export format.

Raw Lottie JSON is the exception: it is both editable source and a portable animation document. Binary `.lottie` bundles remain downloadable but are not expanded into editable JSON automatically.
...
GIFs from Animation and Material are browser-generated. Use them in email and documentation only after checking file size and client support. Preserve the original project settings so a lighter or differently timed export can be regenerated later.
