...
| Image import says browser storage is full | Embedded brand images have filled this origin's local storage quota | In the image or sticker picker, confirm deletion of unused saved assets, then retry; existing artboard layers keep their embedded copy |
...
After changing projects or tools, wait for `glyphfield:studio-api-ready` and re-read `window.glyphfield.studio`. A cached adapter rejects source/control operations when its original editor is no longer active; reacquire the API instead of retrying the stale handle.
