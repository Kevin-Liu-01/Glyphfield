...
| GET | `/api/og` | Parameterized 1200 × 630 OpenGraph PNG |
| GET | `/api/og-home` | Canonical home-page OpenGraph PNG |
| GET | `/og/docs/{slug}/image.png` | Canonical documentation OpenGraph PNG |
...
Frame selection is a browser-native workflow, not a new HTTP generation kind. `design.frame.pause` synchronously pauses native motion and returns `null` without encoding or storing frame assets. `design.frame.capture` persists the visible PNG appearance and returns portable source; `design.frame.play` resumes motion; `design.frame.seek` accepts `{ timeMs }` for seekable providers; `design.motion.describe` reports per-layer capabilities. `/api/materials` includes each material's motion model, PNG capture support, editable seek support, and known or unverified loop behavior. Stateful Fluid history is not seekable.

PNG/JPG export freezes the exact current visible frame. GIF/MP4 sample motion from the current frame anchor and restore that paused pose afterward. Exports leave playback paused and do not add captured-frame assets to IndexedDB. Durable frame capture remains a separate action; unavailable resources and unsupported timestamp-sampled Fluid motion are reported explicitly.
...
OpenGraph route inputs and rendering behavior are covered in [Social previews](/docs/reference/social-previews).

`/openapi.json` uses the same discriminated generation request schema returned by `GET /api/generate`, including exact top-level fields, enums, ranges, string limits, and data-URL constraints. The OpenAPI path inventory includes every public machine route above plus the optional `/api/github-stars` product-chrome utility.
