...
Frame selection is a browser-native workflow, not a new HTTP generation kind. `design.frame.capture` persists the visible PNG appearance and returns portable source; `design.frame.play` resumes motion; `design.frame.seek` accepts `{ timeMs }` for seekable providers; `design.motion.describe` reports per-layer capabilities. `/api/materials` includes each material's motion model, PNG capture support, editable seek support, and known or unverified loop behavior. Stateful Fluid history is not seekable.
