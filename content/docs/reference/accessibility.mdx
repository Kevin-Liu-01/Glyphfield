...
The landing page, Studio, and documentation share one theme preference. By default,
**Auto** follows your operating system, including changes while the site is open.
The theme is applied before the first page paint, without waiting for the editor
or shaders to load. Choosing light or dark overrides Auto across the site and
other open Glyphfield tabs. To follow the system again, choose **Visual customization
→ Theme → Auto** in Studio. Theme changes affect the interface, not your artwork.
If browser storage is unavailable, theme controls still work for the current page;
the choice may not survive a reload.
