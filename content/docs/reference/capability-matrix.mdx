...
| Brand book | Navigable standards document; print/PDF workflow | None; derived from identity | `brand-book.print`; identity source is authoritative |
...
Every navigable Studio tool publishes Browser API control automation scoped to its own active workspace. Every tool except Brand book publishes persistent read/apply source support; opening the Code drawer is not required. Direct artifact actions exist for Brand identity, Brand elements, Moodboard, Animation, Lottie, Design Lab, OpenGraph, Terminal card, Blog, Partnership, and Slides, while `describe().capabilities.exports` identifies any output that remains a Browser control or deterministic HTTP result. Call `describe()` before operation. See [Choose an interface](/docs/agents/choose-interface) and [Studio Browser API](/docs/reference/browser-api).
